...
# 项目内部工具
from scripts.rag.retrieval import twoWay_retrieval
from scripts.tool.prompt import build_docs_prompt


@tool
def search_paper_library(rewritten_query: str) -> str:
    """
    ...
    """
    top_docs = twoWay_retrieval(rewritten_query, vector_top_k=20, rerank_top_k=5)  # 用重写过后的query检索
    docs_prompt = build_docs_prompt(top_docs)  # 把docs组成prompt
    return docs_prompt
