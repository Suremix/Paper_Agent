...
def retrieval_by_vector(query: str, top_k: int) -> list[Document]:
    """
    ...
    """
    # 对query做embedding
    query_embed = encode_model.encode([query])

    # 检索并获取最相关的k个embedding的indices
    _, indices = index.search(query_embed, top_k)  # 获取top_k的索引
...
def twoWay_retrieval(query: str, vector_top_k: int, rerank_top_k: int):
    ...
    vector_docs1 = retrieval_by_vector(query, vector_top_k)
    vector_docs2 = retrieval_by_vector(translate_query(query), vector_top_k)
